...
def adicionar_item(item: str, quantidade: int = 1) -> str:
...
def remover_item(item: str, quantidade: int = 1) -> str:
    p = _buscar_produto(item)
    if p is None:
...
def registrar_endereco(endereco: str) -> str:
    checkout_info["endereco"] = endereco.strip()
    return f"Endereço de entrega registrado: {checkout_info['endereco']}"


def registrar_pagamento(forma: str) -> str:
    checkout_info["pagamento"] = forma.strip()
    return f"Forma de pagamento registrada: {checkout_info['pagamento']}"


def verificar_endereco() -> str:
    """Retorna 'endereco_informado' se o endereço já foi registrado, caso contrário retorna 'endereco_ausente'."""
    if "endereco" in checkout_info and checkout_info["endereco"]:
        return "endereco_informado"
    return "endereco_ausente"


def verificar_pagamento() -> str:
    """Retorna 'pagamento_informado' se a forma de pagamento já foi registrada, caso contrário retorna 'pagamento_ausente'."""
    if "pagamento" in checkout_info and checkout_info["pagamento"]:
        return "pagamento_informado"
    return "pagamento_ausente"


def status_checkout() -> str:
    faltando = [f for f in ("endereco", "pagamento") if f not in checkout_info]
    if not faltando:
        return "checkout_completo"
    labels = {"endereco": "endereço de entrega", "pagamento": "forma de pagamento"}
    return "Falta informar: " + " e ".join(labels[f] for f in faltando)


def fechar_pedido() -> str:
    if not carrinho_ativo:
